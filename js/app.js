...
//fonction qui permet de récupérer les matchs selon le jour selectionné
function handleGamesButton () {
  ...
  const options = {
    ...
    //on lance la requête
    const gamesRequest = fetch (`https://api-nba-v1.p.rapidapi.com/games?date=${theYear+"-"+month+"-"+day}`, options)
    //on récupère la réponse de la requete au format json
    .then( function (data) {return data.json()})
